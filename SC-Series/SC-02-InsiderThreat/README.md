...
A long-tenured employee (Alex Turner) with legitimate credentials began accessing HR and Finance file shares outside their normal scope, downloaded unusually large volumes of data, and attempted exfiltration via personal webmail.
...
### Hypothesis
> Alex Turner is abusing their access to collect and exfiltrate sensitive organizational data. The behavior pattern suggests premeditated intent — the access scope, volume, and exfiltration attempt are inconsistent with their job role.

---

### Step 1 — Confirm Authentication (Okta) — Is This Really Alex Turner?
...
| where ActorDisplayName contains "Alex Turner"
| where EventOriginType == "user.session.start"
| project TimeGenerated, ActorDisplayName, SrcIpAddr, 
          SrcGeoCity, OriginalOutcomeResult
...
**Actual Result:** _(Screenshot placeholder — query results showing clean Okta session)_

**Evidence Summary:**
- ✅ Login from registered device
- ✅ Qatar office IP confirmed
- ✅ MFA passed
- **Conclusion:** Insider confirmed, not account compromise

---

### Step 2 — Map Data Access Scope (SigninLogs) — What Systems Did They Access?
...
SigninLogs
| where TimeGenerated > ago(60d)
| where UserPrincipalName contains "alex.turner"
| where ResultType == "0"
...
**Actual Result:** _(Screenshot placeholder — SigninLogs showing HR/Finance access)_

**Evidence Summary:**
- ✅ Accessed HRConnect (outside role)
- ✅ Accessed FinancePortal (outside role)
- **Conclusion:** Role boundary violation confirmed

---

### Step 3 — Quantify Data Volume (CrowdStrike/DeviceEvents) — What Did They Actually Download?
...
DeviceEvents
| where TimeGenerated > ago(60d)
| where InitiatingProcessAccountName contains "alex.turner"
| where ActionType contains "PowerShell" or ActionType contains "Archive"
| project TimeGenerated, DeviceName, ActionType, InitiatingProcessAccountName, Severity
| sort by TimeGenerated asc
```

**Expected Result:** Data staging activity — large file copy, PowerShell compression commands.

**Actual Result:** _(Screenshot placeholder — CrowdStrike/Defender alert showing PowerShell Compress-Archive)_

**Evidence Summary:**
- ✅ Alert: `"Suspicious PowerShell — Compress-Archive on HR directory"`
- ✅ Large volume archival detected
- **Conclusion:** Staging for exfiltration confirmed

---

### Step 4 — Correlate Behavioral Baseline (7-Day Pattern) — Is This a Spike?
...
| where ActorDisplayName contains "Alex Turner"
| where EventOriginType == "user.authentication.sso"
...
**Actual Result:** _(Screenshot placeholder — 7-day baseline showing spike on incident day)_

**Evidence Summary:**
- ✅ Baseline: 1–2 logins/day
- ✅ Incident day: 12+ logins
- **Conclusion:** Behavioral deviation from baseline confirmed

---

### Step 5 — TP/FP Decision

| Signal | Weight | Status |
|---|---|---|
| Role boundary violation (HR + Finance access) | ✅ Strong | Confirmed |
| 2.3 GB download in 18 minutes | ✅ Strong | Confirmed |
| PowerShell Compress-Archive on HR directory | ✅ Strong | Confirmed |
| Exfiltration attempt to personal Gmail (blocked) | ✅ Strong | Confirmed |
| Authentication from known IP, MFA passed | Insider confirmation | Confirmed |
...
**FP Scenario:** Alex Turner was assigned temporary HR/Finance access for a project and was archiving data for IT migration.
...
### Containment
- [ ] Suspend Alex Turner's account immediately via Okta (disable SSO + all active sessions)
- [ ] Revoke all active tokens and app authorizations
- [ ] Block outbound transfers from Alex Turner's device via Defender for Endpoint
...
- [ ] Capture CrowdStrike/Defender timeline for the endpoint
...
## S5 — Control Failure Analysis

| Control | Expected Behavior | What Failed | Severity |
|---|---|---|---|
| **RBAC / Least Privilege** | User should not access HR/Finance systems | Role assignment too broad — no least-privilege enforcement | Critical |
| **UEBA / Behavioral Analytics** | Alert on access volume anomaly earlier | Alert fired 48 minutes after access began — detection lag | High |
| **DLP Policy** | Block + alert on large file upload to personal email | DLP blocked transfer but did not trigger SOC alert in real-time | High |
| **Privileged Access Review** | Quarterly access reviews | Access scope not reviewed — dormant over-privilege | Medium |

---

## S6 — 🟠 AZ-500 Hardening Recommendations (D1–D4)
...
## S7 — Lessons Learned & Interview Q&A
...
### Closure Criteria
- [ ] User account (Alex Turner) suspended and all sessions terminated
...
### S2 — Investigation Screenshots

**Step 1: Okta Authentication**  
![Okta_Auth_Query](./images/S2-Step1-Okta-Auth.png)

**Step 2: SigninLogs Access Scope**  
![SigninLogs_Access](./images/S2-Step2-SigninLogs.png)

**Step 3: CrowdStrike Data Volume**  
![CrowdStrike_Staging](./images/S2-Step3-CrowdStrike.png)

**Step 4: Behavioral Baseline**  
![Baseline_Spike](./images/S2-Step4-Baseline.png)
